...
const Collections = () => {
  ...
  // ✅ INITIALIZE FROM NAVIGATION STATE (CRITICAL FIX)
  const [category, setCategory] = useState([])
  const [subCategory, setSubCategory] = useState(
    location.state?.subCategory ? [location.state.subCategory] : []
  )

  const [showFilter, setShowFilter] = useState(
    Boolean(location.state?.subCategory)
  )
  const [filterProducts, setFilterProducts] = useState([])
  const [sortType, setSortType] = useState('relavent')
    ...
    <div className="flex flex-col sm:flex-row gap-6 sm:gap-10 pt-10 border-t text-[#6B4E2E]">
      ...
      <div className="min-w-[260px]">
        ...
        >
          {/* CATEGORY */}
          <div className="bg-[#EFE2C6] rounded-xl shadow-sm p-5 mt-4">
            <p className="mb-4 text-sm font-semibold tracking-wide">Categories</p>
            {['Unisex', 'Men', 'Women', 'Kids', 'Footwear'].map(cat => (
          ...
          {/* TYPE */}
          <div className="bg-[#EFE2C6] rounded-xl shadow-sm p-5 mt-5">
            <p className="mb-4 text-sm font-semibold tracking-wide">Type</p>
            {['Topwear', 'Bottomwear', 'Winterwear', 'Footwear'].map(type => (
      ...
      <div className="flex-1">
        <div className="flex justify-between items-center mb-6">
          ...
          <select
            onChange={(e) => setSortType(e.target.value)}
            className="border border-[#6B4E2E] rounded-md text-sm px-3 py-1"
          >
            <option value="relavent">Sort: Relevant</option>
            <option value="low-high">Price: Low to High</option>
            <option value="high-low">Price: High to Low</option>
